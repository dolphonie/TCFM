...
def get_diffusion_models():
    ...
    for base_branch in base_branches:
        ...
        for folder in folders:
            ...
            dist_averages.append(np.mean(file['dist_averages'] / 2428, axis=0))

        dist_averages = np.stack(dist_averages, axis=0)
        ...
        # averages.append(np.mean(dist_averages, axis=1))
        # average_stds.append(dist_avg_std)
...
def plot(ax, averages, average_stds, name, all_colors):
    ...
    ax.plot(np.arange(0, 120), averages[2], linewidth=3, label=f'{name}', color=all_colors[2])
...
def main():
    ...
    plot(ax, cfm_averages, average_stds, "T-CFM (Ours)", all_colors)
    ...
    plt.ylabel("P-High ADE", fontsize=18)
    ...
    plt.savefig('cfm_diffusion_ade_std.png')
