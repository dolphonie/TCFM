...
def main():
    ...
    plot(ax, mins, min_stds, "CADENCE", all_colors)
    
    
    # all_colors = ["#78b24c", "#6581c6", "#edae65"]
    all_colors = ['#e41a1c'] * 3
    cfm_averages, average_stds, mins, min_stds, d_averages_ste = get_cfm_models()
    plot(ax, mins, min_stds, "CFM", all_colors)
    ...
    plt.ylabel("Minimum Average Distance Error (minADE)", fontsize=14)
    plt.xlabel("Prediction Horizon", fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.tight_layout()

    plt.savefig('cfm_diffusion_min_ade.png')
