...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, cond, t, global_cond):
        x_recon = self.predict_start_from_noise(x, t=t, noise=self.model(x=x, t=t, global_cond=global_cond))
    ...
    def p_losses(self, x_start, global_cond, cond, t):
        ...
        x_recon = self.model(x=x_noisy, t=t, global_cond=global_cond)
    ...
    def loss(self, x, global_cond, cond):
        ...
        for k, v in global_cond.items():
            if type(v) is torch.Tensor:
                global_cond[k] = v.to(self.betas.device)
