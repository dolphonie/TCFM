...
class PrisonerRenderer:
    def __init__(self, background=True):
        ...
        if self.background:
            ...
            self._background = imageio.imread('diffuser/utils/no_mountain.png')

            # flip the image on horizontal axis
            self._background = np.flip(self._background, axis=0)
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        #     extent=self._extent, cmap=plt.cm.binary, vmin=0, vmax=1)
        if self.background:
            plt.imshow(self._background, extent = [self.min_x, self.max_x, self.min_y, self.max_y])
...
class SmugglerRenderer(PrisonerRenderer):
    def __init__(self, background = True):
        ...
        if self.background:
            # read in the background image
            self._background = imageio.imread('diffuser/utils/smuggler_background_flipped.png')
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        # colors = plt.cm.jet(np.linspace(0,1,path_length))
        plt.plot(observations[:,0], observations[:,1], c='black', zorder=10)
        plt.scatter(observations[:,0], observations[:,1], c='red', zorder=20)
...
class PrisonerRendererGlobe:
    def __init__(self, bounds = 'language'):
        self._remove_margins = False
        # self.lon_min = -76
        # self.lon_max = -69
        # self.lat_min = 39.45
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        # create map using BASEMAP
            m = Basemap(
                        ...
                        lat_0=(self.lat_max - self.lat_min)/2,
                        lon_0=(self.lon_max-self.lon_min)/2,
                        projection='merc',
                        # projection='aeqd',
                        resolution = None,
                        area_thresh=10000.,
...
class SidotiMPLRenderer(PrisonerRendererGlobe):
    def __init__(self):
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        # m.plot(x, y, marker='.', linestyle='', s=1, markersize=size, c='grey', label='Path')
        plt.scatter(x, y, s=size, c=colors, marker='o', label='Path')
        ...
        # Add axis ticks
        plt.xticks(np.arange(self.lon_min, self.lon_max, step=0.1))
        plt.yticks(np.arange(self.lat_min, self.lat_max, step=0.1))
