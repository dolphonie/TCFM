...
class PrisonerDatasetDetectionsWavelet(torch.utils.data.Dataset):
    def __init__(self, 
                 folder_path, 
                 horizon,
                 normalizer,
    ...
    def unnormalize(self, obs):
        """ 
        input: obs, if obs is of size 2, assume it is already in x, y space
        input: obs, if obs is of size 4, assume it is in wavelet space
        Unnormalize the wavelet to the original x, y space"""
    ...
    def __getitem__(self, idx):
        ...
        # add noise according to dataset
        trajectories = trajectories + self.noise_amplitude * np.random.randn(*trajectories.shape)
